...
from focus_guard.core.blocking.base import BlockingStrategy, BlockingDecision, BlockingReason, ContextAwareBlockingStrategy
from focus_guard.core.domain.models import Domain, Category
from focus_guard.core.domain.domain_utils_new import is_valid_domain
...
class DomainExcluderStrategy(ContextAwareBlockingStrategy):
    """
    ...
    def __init__(self):
        ...
        self._logger = logging.getLogger("core.blocking.strategies.domain_excluder")
    ...
    async def should_block_async(self, domain: Domain) -> BlockingDecision:
        """
        Async version of should_block.
    ...
    async def should_block_with_context_async(self, domain: Domain, context: Dict[str, Any]) -> BlockingDecision:
        """
        Async version of should_block_with_context.
        ...
        Returns:
            A BlockingDecision indicating whether the domain should be blocked.
        """
        return await self.should_block_async(domain)
    
    def _check_domain(self, domain: Domain) -> BlockingDecision:
        """
        Internal method to check if a domain should be blocked.
        ...
        if not domain or not hasattr(domain, 'value') or '@' in domain.value:
        ...
        try:
            ...
            if self._is_whitelisted(domain_value) or getattr(domain, 'category', None) == Category.PRODUCTIVITY:
            ...
            if domain_value in self._excluded_domains:
                return BlockingDecision(
                    should_block=True,
                    reason=BlockingReason.DOMAIN_EXCLUDED,
                    details=f"Domain {domain_value} is in the exclusion list"
                )
            ...
            for i in range(1, len(domain_parts)):
                ...
                if parent_domain in self._excluded_domains:
                    return BlockingDecision(
                        should_block=True,
                        reason=BlockingReason.DOMAIN_EXCLUDED,
                        details=f"Parent domain {parent_domain} is in the exclusion list"
                    )
                    
        except (AttributeError, IndexError, ValueError) as e:
            self._logger.error(f"Error checking domain {getattr(domain, 'value', 'unknown')}: {str(e)}")
            return BlockingDecision(should_block=False, reason=None)
