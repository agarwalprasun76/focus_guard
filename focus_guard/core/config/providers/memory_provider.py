        ...
        with self._lock:
            ...
            if isinstance(current, dict) and parts[-1] in current:
                ...
                for i in range(len(parent_path) - 1, -1, -1):
                    ...
                    for j in range(i):
                        if isinstance(parent_dict, dict) and parent_path[j] in parent_dict:
                            parent_dict = parent_dict[parent_path[j]]
                        else:
                            break
                    
                    # Check if the current level is empty
                    if isinstance(parent_dict, dict) and parent_path[i] in parent_dict:
                        child_dict = parent_dict[parent_path[i]]
                        if isinstance(child_dict, dict) and not child_dict:
                            del parent_dict[parent_path[i]]
                        else:
                            break  # Stop if we find a non-empty dictionary
    ...
    def _collect_paths(self, data: Dict[str, Any], current_path: str, paths: Set[str]) -> None:
        ...
        for key, value in data.items():
            ...
            if isinstance(value, dict):
                # Only recurse for dictionaries, don't add intermediate paths
                self._collect_paths(value, path, paths)
            else:
                # Only add leaf paths (actual configuration values)
                paths.add(path)
