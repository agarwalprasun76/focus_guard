...
from focus_guard.core.coordinator.components.config import ConfigComponent, ConfigChangedEventData
from focus_guard.core.coordinator.events import DefaultEventBus, EventTypes
...
class MockConfigManager(Mock):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self._config_data = {}
        # Add internal attributes that ConfigComponent expects
        self._cache = {}
        self._schemas = {}
        self._providers = {}
...
async def test_get_status(config_manager, component):
    ...
    assert "schemas_registered" in status
    assert "providers_registered" in status
...
async def test_status_consistency(config_manager, event_bus):
    ...
    assert "schemas_registered" in status
    assert "providers_registered" in status
    assert status["providers_registered"] == 0
    
    # After initialization
    await component.initialize()
    status = component.get_status()
    assert status["cache_size"] is not None
    assert status["schemas_registered"] is not None
    assert status["providers_registered"] is not None
