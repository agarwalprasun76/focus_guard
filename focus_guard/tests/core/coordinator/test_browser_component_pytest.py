...
from focus_guard.core.coordinator.components.browser import (
...
class TestBrowserIntegrationComponentPytest:
    ...
    def config_manager(self):
        ...
        # Mock configuration methods with proper return values
        config_values = {
            "browser.polling_interval_seconds": 1.0,
            ...
            "browser_integration.max_retries": 3,
            "browser_integration.retry_delay_seconds": 1.0
        }
        manager.get = MagicMock(side_effect=lambda path, default=None: config_values.get(path, default))
        manager.get_value = MagicMock(side_effect=lambda path, default=None: config_values.get(path, default))
