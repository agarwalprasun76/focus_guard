...
from focus_guard.core.config.providers.database_provider import DatabaseConfigProvider
...
class TestDatabaseConfigProvider(unittest.TestCase):
    ...
    def tearDown(self):
        ...
        try:
            # Try to access a connection and close it explicitly
            import sqlite3
        ...
        # Remove the temporary directory (database will be cleaned up with it)
        try:
            self.temp_dir.cleanup()
        except PermissionError:
            # On Windows, sometimes the database file is still locked
            ...
            try:
                ...
                # If still failing, ignore cleanup errors for tests
                pass
