...
This module contains tests for the ExtensionInstaller class in core.browser.extension.installer.
"""
...
class TestExtensionInstaller:
    ...
    @patch('focus_guard.core.browser.extension.tab_server.is_running')
    ...
    @patch('focus_guard.core.browser.extension.installer.TabServer')
    @patch('focus_guard.core.browser.extension.tab_server.is_running')
    ...
    def test_ensure_tab_server_running_port_conflict(self, mock_is_running, mock_tab_server_class, extension_installer):
        ...
        from focus_guard.core.browser.extension.tab_server import TabServerConfig
        expected_config = TabServerConfig(port=8000)
        mock_tab_server_class.assert_called_once_with(config=expected_config)
    ...
    def test_install_extension_success(self, mock_tab_server_class, extension_installer, mock_extension_manager):
        ...
        # Call the method
        success, user_guide_launched = extension_installer.install_extension(BrowserType.CHROME)
        
        # Verify the result
        assert success is True
        assert isinstance(user_guide_launched, bool)
    ...
    def test_install_for_detected_browsers(self, extension_installer, mock_browser_detector, mock_extension_manager):
        ...
        # Make the extension manager's install_extension method return a boolean
        mock_extension_manager.install_extension.return_value = True
        ...
        with patch.object(ExtensionInstaller, 'ensure_tab_server_running', return_value=True), \
            ...
            # Verify the results
            expected_result = {
                BrowserType.CHROME: {'success': True, 'user_guide_launched': False},
                BrowserType.FIREFOX: {'success': True, 'user_guide_launched': False}
            }
    ...
    def test_install_for_detected_browsers_mixed_results(self, extension_installer, mock_browser_detector, mock_extension_manager):
        ...
        with patch.object(ExtensionInstaller, 'ensure_tab_server_running', return_value=True), \
             patch.object(ExtensionInstaller, 'install_extension', side_effect=[
                 (True, False),  # CHROME: success=True, user_guide_launched=False
                 (False, True)   # FIREFOX: success=False, user_guide_launched=True
            ...
            expected_result = {
                BrowserType.CHROME: {'success': True, 'user_guide_launched': False},
                BrowserType.FIREFOX: {'success': False, 'user_guide_launched': True}
            }
            assert results == expected_result
            
            # Verify install_extension was called for each browser
            assert mock_install_extension.call_count == 2
            mock_install_extension.assert_any_call(BrowserType.CHROME)
            mock_install_extension.assert_any_call(BrowserType.FIREFOX)
