...
todos:
  ...
  - id: phase4-cache
    ...
    status: pending
  - id: phase4-schedule-design
    content: "Phase 4.7: Time-based access schedules -- design data model, ScheduleManager, integrate into ClassificationBlocker, API endpoints"
    status: pending
  - id: phase5-docs
...
- Debug contract mismatch between frontend and `/api/status` response
...
**Design note:** For modularity and auditability of the blocking/classification workflow, see [BLOCKING_CLASSIFICATION_PRIORITY_AND_DESIGN.md](BLOCKING_CLASSIFICATION_PRIORITY_AND_DESIGN.md) (prioritized improvements, step-based pipeline, decision log with step trace).
...
#### 4.7 Time-Based Access Schedules (W-70) -- 3-5 days

A hierarchical scheduling system that controls what content is allowed at different times of day. This is a natural fit for a parental control app -- parents want strict rules during school hours and relaxed rules after school.

**Concept: Schedule Profiles**

The parent defines named "schedule profiles" that map time windows to access policies. Each profile specifies which categories are allowed and what budgets apply during that window.

Example configuration:

- **Focus Time** (Mon-Fri 8:00-15:00): Only EDUCATION and PRODUCTIVITY allowed. All distractions blocked regardless of budget.
- **Homework Time** (Mon-Fri 15:00-17:00): EDUCATION allowed freely. Entertainment/Social allowed but with tighter budgets (e.g., 10 min total).
- **Free Time** (Mon-Fri 17:00-21:00): Normal budget rules apply. Entertainment/Gaming/Social allowed within daily budget.
- **Bedtime** (Every day 21:00-07:00): Everything blocked except system whitelist.
- **Weekend** (Sat-Sun 09:00-21:00): Relaxed budgets (2x normal limits).

**Data Model (new `schedules` key in `domain_config.json`)**
...
```json
{
  "schedules": {
    ...
    "weekly_schedule": {
      "monday": [
        { "start": "00:00", "end": "07:00", "profile": "bedtime" },
        { "start": "07:00", "end": "08:00", "profile": "free_time" },
        { "start": "08:00", "end": "15:00", "profile": "focus_time" },
        { "start": "15:00", "end": "17:00", "profile": "homework_time" },
        { "start": "17:00", "end": "21:00", "profile": "free_time" },
        { "start": "21:00", "end": "23:59", "profile": "bedtime" }
      ]
    },
    "domain_overrides": {
      "khanacademy.org": { "always_allow_in": ["focus_time", "homework_time"] },
      "youtube.com": { "only_allow_in": ["free_time"], "require_educational": true }
    }
  }
}
```

**Integration into blocking pipeline**

The schedule check inserts early in `ClassificationBlocker.check_blocking()`:

```mermaid
flowchart TD
    URL["URL + Domain"] --> AlwaysAllowed["Always-allowed check"]
    AlwaysAllowed -->|"Not always-allowed"| ScheduleCheck["Schedule Check (NEW)"]
    
    ScheduleCheck --> ResolveProfile["Resolve active profile for current time + day"]
    ResolveProfile --> DomainOverride["Check domain_overrides for this domain"]
    
    DomainOverride -->|"Domain explicitly allowed in this profile"| Allow["ALLOW"]
    DomainOverride -->|"Domain explicitly blocked in this profile"| Block["BLOCK"]
    DomainOverride -->|"No domain override"| CategoryCheck["Check classification vs profile.allowed_categories"]
    
    CategoryCheck -->|"Category allowed"| BudgetCheck["Apply budget_multiplier to normal budgets"]
    CategoryCheck -->|"Category blocked"| ProfileBlock["BLOCK (schedule policy)"]
    
    BudgetCheck --> NormalPipeline["Continue normal budget/override pipeline"]
```
...
- Schedules are **hierarchical**: domain_overrides > profile.allowed_categories > profile.blocked_categories > default rules
- `budget_multiplier` scales the normal budget (0 = no budget/blocked, 0.5 = half budget, 2.0 = double)
- `override_allowed: false` means the child cannot request overrides during Focus Time
- Default profile (when no schedule matches): `free_time` (normal rules apply)
- Schedules are per-device, not per-domain (a domain can have overrides within a schedule)

**Implementation steps:**

1. **Backend: Schedule resolution** (1 day)
  - File: new `focus_guard/core/browser_v2/tab_server/schedule_manager.py`
  - `ScheduleManager` class: resolves current profile from day-of-week + time
  - Load schedules from `domain_config.json` via `DomainConfigManager`
  - Method: `get_active_profile() -> ScheduleProfile`
  - Method: `get_domain_schedule_override(domain) -> Optional[DomainScheduleOverride]`
2. **Backend: Integrate into ClassificationBlocker** (0.5 day)
  - File: [classification_blocker.py](focus_guard/core/browser_v2/tab_server/classification_blocker.py)
  - In `check_blocking()`, after always-allowed check, call `schedule_manager.get_active_profile()`
  - Apply profile's allowed/blocked categories before budget check
  - Multiply budget by `budget_multiplier`
  - Return schedule context in blocking response (so blocked page can show "Blocked during Focus Time")
3. **Backend: API endpoints** (0.5 day)
  - `GET /api/schedule` -- return current schedule config + active profile
  - `POST /api/schedule/profiles` -- create/update a profile
  - `POST /api/schedule/weekly` -- update weekly schedule
  - `POST /api/schedule/domain-overrides` -- set per-domain schedule overrides
4. **Frontend: Schedule editor in Settings** (1.5 days)
  - Visual weekly calendar grid (rows = days, columns = hours, cells = colored by profile)
  - Click/drag to paint time slots with a profile
  - Profile editor: name, allowed categories (checkboxes), budget multiplier (slider), override toggle
  - Per-domain schedule popup: accessible from domain management table
5. **Blocked page: Schedule context** (0.5 day)
  - Show "Blocked during Focus Time (ends at 3:00 PM)" instead of generic reason
  - Disable override button when `override_allowed: false`
  - Show countdown to next profile change

**MVP scope vs full feature:**

- MVP: 2-3 predefined profiles (Focus, Free, Bedtime) + simple weekday/weekend split -- no custom calendar UI, just a settings form
- Full: Visual weekly calendar, custom profiles, per-domain overrides, drag-to-paint

#### 4.8 School/Work Calendar Integration (Post-MVP, W-71) -- 1-2 weeks

Extends the time-based schedule system (4.7) by importing schedule data from external calendars instead of requiring manual configuration. This makes the system context-aware: it knows when the child has math class vs free period vs lunch.

**Concept**: Connect to Google Calendar, Outlook, or an iCal feed. Parse events and map them to schedule profiles automatically:
- Events tagged "class", "school", or in a "School" calendar -> Focus Time profile
- Events tagged "homework", "study" -> Homework Time profile
- Gaps between events -> Free Time profile
- After a configurable evening cutoff -> Bedtime profile

**Integration options (pick one for MVP of this feature)**:
- **Option A: iCal URL import** (simplest) -- parent pastes an `.ics` URL from Google Calendar / Outlook. FocusGuard polls it every hour and rebuilds the weekly schedule. No auth needed.
- **Option B: Google Calendar API** -- OAuth2 flow, real-time sync, richer metadata. More complex but supports read-only calendar access with proper permissions.
- **Option C: Manual CSV/JSON import** -- parent exports schedule from school portal and uploads it. Simplest technically but worst UX.

**Recommended**: Start with Option A (iCal URL). Most school portals and Google Calendar support exporting an iCal feed URL. This avoids OAuth complexity while being genuinely useful.

**How it connects to 4.7**: The calendar importer produces the same `weekly_schedule` + `profiles` data structure that the manual schedule editor uses. The parent can then tweak the auto-generated schedule in the UI. Calendar-sourced entries are marked `source: "calendar"` so they can be refreshed without overwriting manual edits.

**Deferred to post-MVP** -- requires the schedule system (4.7) to be built and stable first.
...
**Multi-session (classification pipeline hardening):** Phase 4.3-4.5 (decision log, LLM tracking, user rules) + Phase 4.7 (time-based schedules -- MVP scope: predefined profiles + weekday/weekend split, ~2-3 days; full visual calendar UI deferred to post-MVP)
...
- Auto-update (W-51)
- CI/CD (W-53)
- School/Work calendar integration (W-71) -- depends on time-based schedules (4.7) being stable first
