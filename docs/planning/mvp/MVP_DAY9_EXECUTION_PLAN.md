...
## Tasks
- [ ] Document supported deployment posture in `INSTALL_WINDOWS.md`:
...
- [ ] Add deployment metadata fields (if needed) in `deployment/config.py` for install posture tracking.
- [ ] Ensure installer/runtime code writes/reads posture fields safely (backward compatible defaults).
- [ ] Add an operator checklist for "machine prepared correctly" before first run.
- [ ] Add/adjust tests for config load/save compatibility when new fields are absent.
...
- [ ] Existing installs still load config cleanly.
- [ ] New installs persist posture metadata and show expected defaults.
- [ ] Install docs reflect one unambiguous supported model.
- [ ] Reviewer can answer "who installs?" and "who is monitored?" from docs alone.
