...
## Tasks
- [ ] Add a practical remote login runbook in `INSTALL_WINDOWS.md` for out-of-network access:
...
- [ ] Update `MVP_SMOKE_TEST.md` with a Week 2 remote-access verification step.
- [ ] Run or document evidence for Week 2 validation:
  - setup validation gate behavior
  - extension onboarding clarity
  - metrics range-query checks
  - remote login path test
- [ ] Create/update a Week 2 handoff section (in Week 2 master or a dedicated handoff file).

## Optional stretch
- [ ] Run `python scripts/admin_gateway_smoke.py --password ...` and append one-line result in relevant handoff docs.
- [ ] Cross-link Week 2 outcomes back into `MVP_SPRINT_MASTER_PLAN.md`.

## Validation checklist
- [ ] A new operator can follow docs to access dashboard from a different network safely.
- [ ] Security caveats and guardrails are clear at the point of action.
- [ ] Week 2 execution tracker in master plan reflects actual status.
- [ ] Open follow-ups are moved to parking lot explicitly (no silent leftovers).
