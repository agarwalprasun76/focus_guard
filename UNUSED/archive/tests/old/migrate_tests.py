...
class TestMigrationAnalyzer:
    ...
    def scan_test_files(self) -> List[str]:
        ...
        coordinator_dir = self.base_path / "tests" / "core" / "coordinator"
        test_files = []
...
from core.coordinator.components.{base_name} import {base_name.replace('_', ' ').title().replace(' ', '')}Component
...
def main():
    ...
    for file_path in test_files:
        ...
        print(f"python -m pytest tests/core/coordinator/{migrated} -v  # Migrated")
        print()
