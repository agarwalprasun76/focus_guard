#!/usr/bin/env python
"""
Test script for programmatic browser extension installation.

This script tests the extension installation functionality for Chrome and Edge.
"""
...
def parse_arguments():
    ...
    parser = argparse.ArgumentParser(
        description="Focus Guard Extension Installation Test"
    )
    ...
    parser.add_argument(
        ...
        default="all",
        help="Browser to install extension for"
    )
    
    parser.add_argument(
        "--extension-dir",
        help="Directory containing extension files"
    )
...
def main():
    ...
    logger.info("Starting Focus Guard Extension Installation Test...")
    
    # Use the correct extension directory if not specified
    if not args.extension_dir:
        # Try to find the extension directory
        possible_paths = [
            os.path.join(project_root, 'core_v2', 'browser', 'extension', 'webextension_mv3'),
