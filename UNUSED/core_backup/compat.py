"""
Compatibility layer for transitioning from core to core_v2.

This module provides compatibility functions and classes to ease the transition
from the old core module to the new core_v2 module. It allows for gradual
...
from core_v2.api import api
from core_v2.domain.models import Category, Domain
...
def map_category_to_legacy(category: Optional[Category]) -> Optional[DomainCategory]:
    """
    Map a core_v2 Category to a legacy DomainCategory.
    
    Args:
        category: The core_v2 Category to map.
...
def classify_domain(domain: str) -> Optional[DomainCategory]:
    """
    Classify a domain using the new core_v2 API but return a legacy DomainCategory.
...
def should_block_tab(url: str, metadata: Optional[Dict[str, Any]] = None) -> bool:
    """
    Determine if a tab should be blocked using the new core_v2 API.
...
def get_blocking_reason(url: str) -> Optional[str]:
    """
    Get the reason why a tab is blocked using the new core_v2 API.
...
def reload_configuration() -> None:
    """
    Reload the configuration using the new core_v2 API.
...
class DomainClassifierCompat:
    """
    Compatibility class that mimics the legacy DomainClassifier but uses the new core_v2 API.
    """
...
class BlockingManagerCompat:
    """
    Compatibility class that mimics the legacy BlockingManager but uses the new core_v2 API.
    """
