...
This module provides a classifier for YouTube domains and content.
"""
...
from focus_guard.core.domain.models import Domain, Category, URL, ClassificationResult


class YouTubeClassifierAdapter(ContextAwareClassifier):
    """
    Classifier for YouTube domains and content.
    
    This class classifies YouTube domains and can also classify
    specific YouTube content based on URL patterns.
    """
    ...
    # YouTube URL patterns for different content types
    YOUTUBE_VIDEO_PATTERN = re.compile(r'(?:v=|youtu\.be\/|embed\/|shorts\/)([^\&\?\/]+)')
    YOUTUBE_SHORTS_PATTERN = re.compile(r'shorts\/([^\&\?\/]+)')
    YOUTUBE_PLAYLIST_PATTERN = re.compile(r'[&?]list=([^&]+)')
    
    def __init__(self):
        """Initialize the YouTube classifier."""
        self._logger = logging.getLogger(__name__)
        self._logger.info("YouTube classifier initialized")
