        ...
        try:
            while True:
                try:
                    ...
                    window_info = self._activity_monitor.get_active_window_info()
                    
                    # Check if window changed
                    if (self._last_window_info is None or 
                            window_info.window_id != self._last_window_info.window_id or
                            window_info.title != self._last_window_info.title or
                            window_info.process_name != self._last_window_info.process_name):
                        
                        # Publish window changed event
                        await self._event_bus.publish(
                            EventTypes.WINDOW_CHANGED,
                            WindowChangedEventData("activity_monitor", window_info.title, window_info.process_name, window_info.pid)
                        )
                    ...
                    if is_idle != self._is_idle:
                        ...
                        await self._event_bus.publish(
                            ...
                            IdleStateChangedEventData("activity_monitor", is_idle, idle_time)
                        )
