...
copy "%~dp0focus_guard_native_host.exe" "%FG%" >nul

REM 2 ▸ register native-messaging host (Chrome family)
set MAN=%LOCALAPPDATA%\FocusGuard\focus_guard_host.json
> "%MAN%" (
  echo {
  echo   "name": "com.focusguard.native",
  echo   "description": "FocusGuard native bridge",
  echo   "path": "%LOCALAPPDATA:\=\\\\%\\FocusGuard\\focus_guard_native_host.exe",
  echo   "type": "stdio",
